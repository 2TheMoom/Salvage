...
- **Arc Network support:** Circle's Arc mainnet launches September 16, 2026. Turned out to be more than "a new chain config" — Arc isn't in Etherscan's V2 chainlist, so the scanner's entire ABI/verification pipeline needed a second path routing to Arcscan (a separate Blockscout instance; confirmed its legacy API is deliberately Etherscan-compatible, same response shape, no key needed). Full groundwork is done and verified against Arc Testnet, not just local Hardhat — Arc's own docs warn that local EVM simulators can't reproduce its real behavior (native-USDC gas, blocklist enforcement, minimum base fee) — including a genuine end-to-end register → send → settle run on real Arc infrastructure with every balance independently confirmed on-chain. App-side wiring (chain selector, wallet config, explorer links across every chain-aware surface) is in and typechecks clean. What's left: swapping testnet values (chain ID, RPC, contract addresses) for confirmed mainnet ones once Arc publishes them on the 16th.
