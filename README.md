...
Ethereum and Base contracts verified on **Etherscan/Basescan, Blockscout, and Sourcify**. Arc's contracts aren't source-verified anywhere yet — no confirmed Blockscout-compatible explorer API exists for Arc mainnet at launch, so there's nowhere to submit to. Their correctness is instead independently confirmed by direct RPC reads against the deployed bytecode (`owner()`, `protocolFeeRecipient()`, the wrapper's `router()` pointer) matching expected values. The Fee Contract was an earlier design (manual founder-confirmed recoveries, ETH-denominated fees) fully superseded by the Router's permissionless, signature-based settlement — the app no longer calls it, and it never held stranded ERC-20 tokens.
...
- **✅ Arc Network mainnet support:** Circle's Arc mainnet launched September 16, 2026 — same day, live. Turned out to be more than "a new chain config": Arc isn't in Etherscan's V2 chainlist, so the scanner's ABI-verification pipeline needed its own path for Arc. No confirmed Blockscout-compatible explorer API exists for Arc mainnet yet (the testnet-era "Arcscan" instance has no mainnet counterpart Circle documents or that resolves), so that path fails closed — Arc contract scans honestly report "not verified" rather than guess at an API URL, everything else (balance sweeps, pricing, recovery, settlement) works normally. Chain ID, RPC, and explorer values were confirmed directly against Circle's own docs rather than secondary sources — a third-party blog had the right chain ID but a wrong RPC URL, caught by cross-checking. `SalvageRecoveryRouter` and `SalvageBatchWrapper` are deployed on Arc mainnet, each independently verified via direct RPC reads (bytecode, `owner()`, `protocolFeeRecipient()`, the wrapper's `router()` pointer) before their addresses went into the app.
